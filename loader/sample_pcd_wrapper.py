...
class RealPCDSequenceSampler:
    ...
    def __init__(
        ...
        noise_extrinsic_parameter=True,
    ...
    def load_pcd(self, data):

        euler_angles = (np.random.rand(3) * 2 -
                        1) * self.noise_extrinsic_parameter[-1]
        ...
        translation = (np.random.rand(3) * 2 -
                       1) * self.noise_extrinsic_parameter[0]

        shuffle_data[
            0, :, :3] = shuffle_data[0, :, :3] @ rotation_matrix + translation

        return shuffle_data.transpose(0, 2, 1)
        ...
        for key in self.keys:
            ...
            if key not in self.key_first_k:
                if key == "seg_pc":
                    ...
                    sample = self.load_pcd(
                        input_arr[buffer_start_idx:buffer_end_idx])
